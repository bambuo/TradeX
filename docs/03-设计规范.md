...
flowchart TB
  ...
  TRADEX -->|"EF Core (Pomelo MySQL)"| MYSQL
  TRADEX -.->|"Redis Stream 事件桥接"| REDIS
  WORKER -->|"EF Core (Pomelo MySQL)"| MYSQL
...
| MySQL | MySQL 8.4 + Pomelo EF Core | 1 | 持久卷 | N/A |
...
services:
  mysql:
    image: mysql:8.4
    container_name: tradex-mysql
    environment:
      - MYSQL_ROOT_PASSWORD=${MYSQL_ROOT_PASSWORD:?MYSQL_ROOT_PASSWORD is required}
      - MYSQL_DATABASE=tradex
    volumes:
      - tradex-mysql-data:/var/lib/mysql
    healthcheck:
  ...
  backend:
    ...
    environment:
      ...
      - ConnectionStrings__DefaultConnection=Server=mysql;Port=3306;Database=tradex;User Id=tradex;Password=${MYSQL_PASSWORD};SslMode=Preferred;
...
| `ConnectionStrings__DefaultConnection` | — | MySQL 连接串 |
...
| `MYSQL_PASSWORD` | — | MySQL tradex 用户密码 |
