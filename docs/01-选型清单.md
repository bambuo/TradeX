...
| 技术 | 版本 | 决策 | 决策理由 | 备选方案 |
|------|------|------|----------|----------|
| PostgreSQL | 16 | ✅ 选用 | 成熟的关系型数据库，ACID 事务保证数据一致性；Npgsql EF Core 驱动在 `net10.0` 上稳定，迁移管理完善；多进程并发写入无文件锁风险；`pg_dump` 备份简单；Docker 容器化部署统一管理 | MySQL（同样成熟，但 Pomelo 驱动生态不如 Npgsql 活跃）；SQLite（更适合嵌入式场景，多进程并发写入有文件锁风险） |
...
| Docker Compose | ✅ 选用 | 单机编排，`depends_on` 管理启动顺序，持久卷映射 PostgreSQL 数据 + Redis；`networks` 隔离内网通信 | K8s（复杂度不合算）；`docker stack`（需要 Swarm 模式，功能不如 Compose 直观） |
...
| `ConnectionStrings__DefaultConnection` | Host=postgres;Port=5432;Database=tradex;Username=tradex;... | PostgreSQL 连接串 |
...
### ADR-001: PostgreSQL + Redis 为主存储方案

| 属性 | 值 |
|------|-----|
| **状态** | ✅ **已采纳** |
| **上下文** | TradeX 为多进程 Docker 部署系统（API + Worker），需支持两个进程并发写入，同时需要跨进程事件总线 |
| **决策** | 使用 PostgreSQL 为主存储引擎（EF Core + Npgsql），Redis 为跨进程事件总线 / 缓存 |
| **理由** | PostgreSQL 支持多进程并发写入无文件锁；Npgsql 驱动在 `net10.0` 上稳定、迁移管理完善；`pg_dump` 备份机制成熟；Redis Pub/Sub 满足跨进程通信需求 |
| **后果** | 增加 PostgreSQL 和 Redis 容器依赖，部署复杂度较单体 SQLite 提升；需管理数据库连接池 |
...
|------|-----|
| **状态** | ✅ **已采纳** |
| **上下文** | API Key / Secret / Passphrase / MFA Secret 需加密存储在 PostgreSQL 中 |
...
├── TradeX.Infrastructure/ # EF Core + PostgreSQL + Casbin
