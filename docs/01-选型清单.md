...
| C3 | MySQL 为主存储，Redis 为缓存/事件总线 | PRD FR-13 |
...
| 技术 | 版本 | 决策 | 决策理由 | 备选方案 |
|------|------|------|----------|----------|
| Vue 3 + TypeScript + Vite | 8.x | ✅ 选用 | 组合式 API `<script setup>` 渲染模式，Vite 8 极速 HMR，TypeScript 6 类型安全；与后端 SignalR 通过 `@microsoft/signalr` npm 包集成；`Arco Design Vue` 企业级组件库 | Blazor Server（与后端紧密耦合，多语言团队协作成本高）；React（生态更丰富但组件库选择需额外评估） |
| Arco Design Vue | 2.58.0 | ✅ 选用 | 企业级 UI 组件库，50+ 组件覆盖表格/表单/弹窗/导航等全部需求；响应式布局，支持 SSR；字节跳动出品，维护活跃 | Element Plus（社区更老，但在本次 Vue 3 + SSR 场景下 Arco 配置更轻量） |
| Pinia | 3.x | ✅ 选用 | Vue 3 官方推荐状态管理，TypeScript 原生支持；模块化 Store 按业务域拆分，Tree-shakable | Vuex 4（已进入维护模式，Pinia 是其官方替代） |
...
| 技术 | 版本 | 决策 | 决策理由 | 备选方案 |
|------|------|------|----------|----------|
| MySQL | 8.4 | ✅ 选用 | 成熟的关系型数据库，ACID 事务保证数据一致性；EF Core Pomelo 驱动成熟，迁移管理完善；`mysqldump` 备份简单；Docker 容器化部署统一管理 | PostgreSQL（功能同样强大，但 EF Core 驱动在 `net10.0` 上兼容性待验证）；SQLite（更适合嵌入式场景，多进程并发写入有文件锁风险） |
| EF Core | 10.0 | ✅ 选用 | 成熟 ORM，Code-First Migration 自动管理 Schema 版本；与 ASP.NET Core 同属微软生态，集成无缝；LINQ 强类型查询编译期检查 | Dapper（轻量但需手写 SQL，Migration 需额外工具管理） |
| Redis | 7.4 | ✅ 选用 | 跨进程事件总线（Pub/Sub）、SignalR Backplane、分布式锁（未来需要时）；StackExchange.Redis 官方成熟客户端；与 Docker 单机部署无缝集成 | RabbitMQ（功能更强但更重，Pub/Sub 场景 Redis 更轻量）；内存队列（跨进程不可用） |

### 5.1 数据存储分布策略

| 数据类别 | 存储引擎 | 访问模式 | 一致性要求 | 备份策略 |
|----------|---------|---------|-----------|---------|
| 用户 / 配置 | MySQL | 低频读写 | 强一致 | mysqldump |
| 交易所凭证 | MySQL (AES 加密) | 低频读 | 强一致 | 同上 |
| 策略 / 订单 | MySQL | 中频读写 | 强一致 | 同上 |
| 持仓 | MySQL | 高频更新（15s/cycle） | 最终一致 | 同上 |
| 审计日志 | MySQL | 仅追加写 + 低频范围查 | 最终一致 | 同上 + 归档 |
| K 线历史 | 交易所 REST API | 回测时实时拉取 | 最终一致 | — |
| 归档订单 | JSON (gzip) | 极低频读 | 最终一致 | 与 MySQL 同卷 |
...
| Backend ↔ MySQL/Redis | 可信内网 | Docker 内网通信 |
...
| bUnit | 最新 | ❌ 已废弃（非 Blazor 项目） | 原为 Blazor 组件测试框架，项目已迁移至 Vue 3 SPA，前端测试使用 Vitest + Vue Test Utils | — |
| Playwright | 最新 | ✅ 选用 | 跨浏览器 E2E 测试（Chromium/Firefox/WebKit），支持 Vue SPA 的前端测试场景 | Cypress（仅 JavaScript，付费功能较多）；Vitest（单元测试用，不具备 E2E 浏览器交互能力） |
...
| Docker Compose | ✅ 选用 | 单机编排，`depends_on` 管理启动顺序，持久卷映射 MySQL 数据 + Redis；`networks` 隔离内网通信 | K8s（复杂度不合算）；`docker stack`（需要 Swarm 模式，功能不如 Compose 直观） |
...
Vue SPA 前端构建输出由 API 进程的 `wwwroot` 托管，与 REST API / SignalR Hub 在同一端口提供服务。
...
| `ConnectionStrings__DefaultConnection` | Server=mysql;Port=3306;Database=tradex;... | MySQL 连接串 |
| `Redis__ConnectionString` | redis:6379 | Redis 连接串 |
| `Database__MySqlServerVersion` | 8.4.0 | MySQL 版本 |
...
| 场景 | 行为 | RTO | RPO |
|------|------|-----|-----|
| 进程崩溃 | Docker restart + Reconciliation 恢复 | < 30s | 0（MySQL 事务写后即持久化）|
...
### ADR-001: MySQL + Redis 为主存储方案

| 属性 | 值 |
|------|-----|
| **状态** | ✅ **已采纳** |
| **上下文** | TradeX 为多进程 Docker 部署系统（API + Worker），需支持两个进程并发写入，同时需要跨进程事件总线 |
| **决策** | 使用 MySQL 为主存储引擎（EF Core + Pomelo），Redis 为跨进程事件总线 / 缓存 |
| **理由** | MySQL 支持多进程并发写入无文件锁；`mysqldump` 备份机制成熟；Redis Pub/Sub 满足跨进程通信需求 |
| **后果** | 增加 MySQL 和 Redis 容器依赖，部署复杂度较单体 SQLite 提升；需管理数据库连接池 |
| **替代否决** | SQLite（多进程并发写入有文件锁风险，不适合多容器架构） |
...
|------|-----|
| **状态** | ✅ **已采纳** |
| **上下文** | API Key / Secret / Passphrase / MFA Secret 需加密存储在 MySQL 中 |
...
| **理由** | (1) camelCase 是 JSON/Web API 社区标准（JavaScript/TypeScript 均采用），前端可直接使用 `response.dataField` 无需映射；(2) ASP.NET Core `AddJsonOptions(o => o.JsonSerializerOptions.PropertyNamingPolicy = JsonNamingPolicy.CamelCase)` 一行配置即可全局生效；(3) 匿名对象和 DTO 均自动转换，无需逐个加 `[JsonPropertyName]` |
| **实施方式** | (1) 后端 `Program.cs` 中 `AddJsonOptions` 配置 `PropertyNamingPolicy = JsonNamingPolicy.CamelCase`；(2) 手动 `JsonSerializer.Serialize` 处（如 `BacktestEngine.cs`）显式传入相同 options；(3) 前端 API 接口定义使用 camelCase 字段名 |
| **例外** | Controller 查询参数（`[FromQuery]`）不受影响，仍可按 C# 命名 |
| **后果** | 所有 HTTP JSON 响应字段均为小驼峰；新增 API 端点时前后端字段命名需保持一致 |
| **替代否决** | PascalCase（C# 默认，前端需要额外映射层 `.Select(x => new { x.Field })`）；snake_case（Python 友好但 JSON/TypeScript 社区不占优势，与 ASP.NET Core 默认配置不符） |
...
├── TradeX.Infrastructure/ # EF Core + MySQL + Casbin
