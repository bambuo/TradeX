...
// 错误
{ "code": 1000, "message": "描述", "traceId": "00-xxx..." }
// code 为数字类型业务错误码，见 BusinessErrorCode 枚举
```
...
#### `POST /api/v1/auth/send-recovery-codes`
- Auth: **Bearer**（Admin+）
- 说明：重新生成恢复码（旧码立即失效）
- Request:
```json
...
#### `GET /api/v1/exchanges/:id/assets`
- Role: **admin / super_admin / operator**
- 说明：实时获取交易所账户资产余额
- Response `200`:
```json
...
#### `GET /api/v1/traders/{traderId}/strategies`
- Role: **all authenticated**
- 说明：获取交易员下的策略部署列表
- Response `200`:
```json
...
#### `POST /api/v1/traders/{traderId}/strategies`
- Role: **admin / operator**
- 说明：创建策略部署（状态为 `Draft`）
- Request:
```json
...
#### `PUT /api/v1/traders/{traderId}/strategies/{id}`
- Role: **admin / operator**
- 说明：更新策略部署
...
#### `DELETE /api/v1/traders/{traderId}/strategies/{id}`
- Role: **admin / operator**
...
#### `POST /api/v1/traders/{traderId}/strategies/{id}/toggle`
- Role: **admin / operator**
- 说明：启用/停用策略部署
- Request:
```json
...
#### `GET /api/v1/traders/{traderId}/positions`
- Role: **all authenticated**
...
#### `GET /api/v1/traders/{traderId}/positions/{id}`
- Role: **all authenticated**
- 说明：单个持仓详情
...
#### `GET /api/v1/traders/{traderId}/orders`
- Role: **all authenticated**
- 说明：订单列表
- Response `200`:
```json
...
#### `GET /api/v1/traders/{traderId}/orders/{id}`
- Role: **all authenticated**
- 说明：单个订单详情
- Response: 单个 Order 对象

#### `POST /api/v1/traders/{traderId}/orders/manual`
- Role: **admin / operator**
- 说明：手动下单（需通过风控检查）
- Request:
```json
...
- Query: `?format=csv|json&startAt=&endAt=`
...
#### `GET /api/v1/traders/{traderId}/strategies/{strategyId}/backtests/tasks`
- Role: **all authenticated**
- 说明：按 StrategyId 查询回测任务列表
- Response: `200` + `BacktestTask[]`

#### `POST /api/v1/traders/{traderId}/strategies/{strategyId}/backtests`
- Role: **admin / operator**
- 说明：提交回测（异步执行，通过后关联部署状态 `Draft` → `Passed` 仅首次生效）
- Query: `?deploymentId=guid&exchangeId=guid&Pair=string&timeframe=string&startAt=datetime&endAt=datetime&initialCapital=1000`
- Response `200`:
```json
{
  "taskId": "guid",
...
#### `GET /api/v1/traders/{traderId}/strategies/{strategyId}/backtests/tasks/{taskId}/result`
- Role: **all authenticated**
- 说明：获取回测结果
- Response `200`:
```json
...
#### `GET /api/v1/traders/{traderId}/strategies/{strategyId}/backtests/tasks/{taskId}/analysis`
- Role: **all authenticated**
- 说明：分页 K 线分析数据（含指标值）
- Query: `?page=1&pageSize=100`
- Response: 分页数据，使用 `JsonElement` 保留原始 JSON 结构

#### `GET /api/v1/traders/{traderId}/strategies/{strategyId}/backtests/tasks/{taskId}/analysis/stream`
- Role: **all authenticated**
...
- Role: **admin / super_admin**
- 说明：审计日志列表（翻页，支持筛选）
- Query: `?userId=guid&action=user.login&resourceType=exchange&startAt=&endAt=`
- Response `200`:
```json
{
  "data": [
...
{
    /// <summary>评估单个条件节点是否满足（已废弃：旧 ConditionTreeEvaluator 架构）</summary>
    [Obsolete("使用 IStrategyDecisionEngine.Decide 替代")]
    bool Evaluate(ConditionNode node, List<Kline> klines);

    /// <summary>递归评估完整条件树，返回最终 true/false（已废弃：旧 ConditionTreeEvaluator 架构）</summary>
    [Obsolete("使用 IStrategyDecisionEngine.Decide 替代")]
    bool EvaluateTree(ConditionTreeRoot root, List<Kline> klines);
}
```
...
> **架构说明**：`IExchangeClient` 已拆分为 4 个能力接口，消费方可按需注入更窄接口。
...
{
    Task<TResult> CalculateAsync(List<Kline> klines, Dictionary<string, object> parameters);
}
```
...
> `code` 字段为数字类型，定义于 `TradeX.Core.ErrorCodes.BusinessErrorCode`。
...
| `1100` | 401 | 用户名或密码错误 |
| `1101` | 403 | 用户已被禁用 |
| `1102` | 401 | 需要 MFA 验证（请求头缺少 X-MFA-Code） |
| `1103` | 401 | MFA 验证码或恢复码错误 |
| `1104` | 403 | 当前账户未启用 MFA |
| `1105` | 401 | MFA 重放检测拒绝 |
| `1106` | 500 | MFA 密钥无效 |
| `1107` | 401 | RefreshToken 无效或已吊销 |
| `1108` | 409 | 用户名已存在 |
| `1109` | 400 | 角色分配非法 |
...
|------|------|------|
| `1200` | 404 | 交易所配置不存在 |
| `1201` | 400/502 | 连接测试失败或获取数据失败 |
| `1202` | 400 | 交易规则拉取失败 |
| `1203` | 409 | 有关联活跃策略，不可删除 |
...
| `1301` | 400 | 当前状态不可编辑 |
| `1302` | 400 | 策略不满足启用条件 |
| `1303` | 409 | 同一 Trader 在同一 Exchange 上对同一 Pair 已有活跃策略 |
| `1304` | 409 | 活跃策略不可删除 |
| `1305` | 400 | K 线预热失败 |
...
|------|------|------|
| `1800` | 404 | 用户不存在 |
| `1801` | 400 | 不可禁用 Super Admin |
| `1802` | 400 | 不可降级 Super Admin |
...
```
p, super_admin, /api/v1/*, (GET)|(POST)|(PUT)|(DELETE)
p, admin, /api/v1/traders/*, (GET)|(POST)|(PUT)|(DELETE)
p, admin, /api/v1/traders/*/strategies/*, (GET)|(POST)|(PUT)|(DELETE)
p, admin, /api/v1/traders/*/strategies/*/toggle, POST
p, admin, /api/v1/exchanges/*, (GET)|(POST)|(PUT)|(DELETE)
p, admin, /api/v1/exchanges/*/test, POST
p, admin, /api/v1/exchanges/*/toggle, POST
p, admin, /api/v1/exchanges/*/Pairs, GET
p, admin, /api/v1/exchanges/*/assets, GET
p, admin, /api/v1/exchanges/*/orders, GET
p, admin, /api/v1/users/*, (GET)|(POST)|(PUT)
p, admin, /api/v1/audit-logs, GET
p, admin, /api/v1/system/*, (GET)|(PUT)|(POST)
p, admin, /api/v1/notifications/*, (GET)|(POST)|(PUT)|(DELETE)
p, admin, /api/v1/settings, (GET)|(PUT)
p, operator, /api/v1/traders, (GET)|(POST)
p, operator, /api/v1/traders/*, (GET)|(PUT)
p, operator, /api/v1/traders/*/strategies, (GET)|(POST)
p, operator, /api/v1/traders/*/strategies/*, (GET)|(PUT)
p, operator, /api/v1/traders/*/strategies/*/toggle, POST
p, operator, /api/v1/traders/*/orders, GET
p, operator, /api/v1/traders/*/orders/manual, POST
p, operator, /api/v1/traders/*/positions, GET
p, operator, /api/v1/exchanges/*, (GET)|(POST)|(PUT)|(DELETE)
p, operator, /api/v1/exchanges/*/test, POST
p, operator, /api/v1/exchanges/*/toggle, POST
p, operator, /api/v1/exchanges/*/Pairs, GET
p, operator, /api/v1/exchanges/*/assets, GET
p, operator, /api/v1/exchanges/*/orders, GET
p, operator, /api/v1/strategies, (GET)|(POST)
p, operator, /api/v1/strategies/*, (GET)|(PUT)|(DELETE)
p, operator, /api/v1/dashboard/*, GET
p, operator, /api/v1/backtests, (GET)|(POST)
p, operator, /api/v1/orders/export, GET
p, viewer, /api/v1/*, GET
```
...
2. 未初始化 → 仅放行 `/health` + `/api/v1/setup/*`
