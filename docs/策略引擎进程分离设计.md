...
### 1.2 非目标
- **不引入消息中间件集群**：当前单机部署继续使用 MySQL 8.4 + Redis 7.4，不引入 Kafka/RabbitMQ。
...
API 与 Worker 均连接同一个 MySQL 实例。业务事件采用 outbox 表作为事务边界：业务写入提交后，Worker 内的 `OutboxRelayService` 将事件推送到 Redis Stream，API 再桥接给 SignalR。
...
| 风险 | 缓解 |
|------|------|
| API 与 Worker 并发写 MySQL | 关键写路径使用乐观并发、唯一索引和 Outbox 事务边界；需要持续补充集成测试 |
...
2. **是否进一步收敛写路径**：当前 API 与 Worker 都写 MySQL；若未来需要更强串行语义，可评估把交易相关写操作统一收敛到 Worker。
