...
>
> **更新说明**：本文档记录的设计已全量实现，包括 Redis 事件总线、Outbox 模式、命令通道等。
...
**关键耦合点（已解耦）**：
1. ~~`SignalREventBus` 直接持有 `IHubContext<TradingHub>`~~ → 已替换为 `OutboxTradingEventBus` + `OutboxRelayService`
2. ~~`MarketDataCache` 是内存单例~~ → 已完全移到 Worker
3. ~~`BacktestTaskQueue` 内存队列~~ → 已改为 Redis Stream + `BacktestTaskListener`
4. ~~所有组件共享同一 EF Core `IServiceScopeFactory`~~ → API 与 Worker 各自独立 Scope
...
| 用户启用/禁用策略 | API → Worker | 低 | **Redis Stream 命令通道**（`RefreshSubscriptions` 命令，秒级延迟） |
| 用户修改风控参数 | API → Worker | 低 | **Redis Stream 命令通道**（同上） |
| 用户手动触发回测 | API → Worker | 中 | **Redis Stream 命令通道**（`tradex:backtest` Stream） |
...
// RedisToSignalRBridge : BackgroundService（已实现）
//   - XREADGROUP "tradex:events" → hub.Clients.Group(traderId).SendAsync(...)
```

**已实现**：Worker 进程只写 outbox，后台 relay 发布 Redis Stream。API 进程消费 Redis Stream，桥接到 SignalR。
...
**已实现**：当前 `PriceHistory` 和 `LastTradeTime` 完全移到 Worker，API 进程不再持有 `MarketDataCache`。
...
| `TradeX.BacktestWorker` | 新增独立回测 Worker 进程，与策略 Worker 隔离 | ✅ |
...
- 定义命令模型：`tradex:cmd` Stream
- Worker 订阅命令通道
- API 通过 Redis Stream 发命令
- 现有功能不依赖此通道，仅作为增量能力
...
**所有阶段已完成**。当前架构稳定运行，建议持续关注可观测性和性能优化。
