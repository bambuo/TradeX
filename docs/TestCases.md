...
| AC-16: 回测运行+绩效报告 | TC-BACKTEST-001 ~ TC-BACKTEST-020 |
...
### TC-DATA-003: K 线数据同时写入 IoTDB
- **前置条件**: IoTDB 可连接
- **测试步骤**:
  1. 接收 K 线数据后查询 IoTDB
- **预期结果**: IoTDB 对应序列路径下有新写入的数据
- **关联需求**: FR-05.4, TAD §4.3

### TC-DATA-004: IoTDB 不可用时降级
- **前置条件**: IoTDB 连接断开
- **测试步骤**:
  1. 模拟 IoTDB 不可用
  2. 检查系统行为
- **预期结果**: 数据继续写入内存缓存，不崩溃，日志记录 IoTDB 写入失败，不阻塞策略评估
- **关联需求**: FR-05.3, TAD §9.2
...
- **测试步骤**:
  1. WS 恢复连接
  2. 检查回填逻辑
- **预期结果**: 通过 REST API 回填缺失区间的 K 线数据到内存缓存和 IoTDB（去重）
...
- **测试步骤**:
  1. 执行 toggle enable
- **预期结果**: 自动从交易所 REST 拉取至少 3 天的 15 分钟级 K 线数据到 IoTDB
...
### TC-DATA-012: K 线预热 — 数据去重
- **前置条件**: 部分 K 线数据已存在 IoTDB
- **测试步骤**:
  1. 执行预热，检查重复数据
- **预期结果**: 预热不产生重复 K 线记录（幂等写入）
- **关联需求**: TAD §5.3
...
- **预期结果**: 所有服务启动成功（tradex-api, tradex-db, tradex-iotdb 等），状态为 healthy
