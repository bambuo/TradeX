...
| 前端框架 | Vue 3 + TypeScript + Pinia | Vite 构建，内嵌至 ASP.NET Core 同一进程 |
...
| ID | 需求 |
|----|------|
| FR-16.1 | 根级统一 Dockerfile，3 阶段构建：Node 构建前端 → .NET SDK 构建后端 → ASP.NET Runtime 单容器运行 |
| FR-16.2 | 前端 SPA 由 ASP.NET Core 内嵌静态文件服务提供，无需独立 Nginx 容器 |
