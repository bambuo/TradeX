...
| 层 | 技术 | 说明 |
|---|---|---|
| 后端框架 | ASP.NET Core 10 + C# 14 | REST API + SignalR + BackgroundService + 静态文件服务 |
| 前端框架 | Blazor Server (InteractiveServer) + Fluent UI | `Microsoft.FluentUI.AspNetCore.Components` v5.0，内嵌至 ASP.NET Core 同一进程 |
...
| 部署 | Docker + Docker Compose | 单容器运行 API+SPA，IoTDB 独立容器 |
...
| ID | 需求 |
|----|------|
| FR-03.1 | 策略模板支持三个部署作用域（优先级从高到低）：**交易对策略（Symbol）→ 交易所策略（Exchange）→ 交易员策略（Trader）**。具体生效规则见 FSD §5.3.1 |
| FR-03.2 | **同一 Trader 在同一 Exchange 上，对同一个 Symbol，同时仅允许 1 个 Active 策略**。尝试启用冲突策略时前端应提示冲突 |
...
| FR-03.11 | 系统必须支持 **Volatility Grid（波幅触发均价再平衡）** 策略风格：当 K 线波幅超过阈值触发首单，后续围绕持仓均价按百分比进行分批加仓/减仓 |
| FR-03.12 | Volatility Grid 必须支持双周期部署能力（`5m` 与 `15m`），可通过多部署协同执行同一策略模板 |
| FR-03.13 | Volatility Grid 必须支持金字塔加仓次数上限（默认 5 次），超过上限不得继续追加仓位 |
| FR-03.14 | Volatility Grid 允许关闭单笔止损，但必须保留账户级风控兜底（如日亏限制、最大回撤） |
...
| FR-07.13 | **币种级风控**：按 Symbol 维度配置总持仓上限、日交易次数上限 |
...
| FR-16.3 | docker-compose.yml 编排：tradex（API+SPA 统一容器）、iotdb，配合持久卷 |
| FR-16.4 | 支持通过环境变量注入关键配置（数据库路径、IoTDB 连接串等） |
| FR-16.5 | 后端必须提供 `GET /health` 端点，返回数据库连接、IoTDB 连接、各交易所 WS 连接状态 |
| FR-16.6 | docker-compose.yml 中为 tradex 和 iotdb 配置 healthcheck 指令 |
...
初始化仅创建 Super Admin（状态 `PendingMfa`），MFA 绑定在首次登录后完成。Admin 创建的用户也是同样流程。

```
初始化或 Admin 创建用户 (status: PendingMfa)
  → 用户首次登录 → 系统返回 mfaToken
  → 调用 POST /api/auth/mfa/setup → 生成 TOTP Secret + QR Code URI + 恢复码
  
用户扫码绑定 Google Authenticator / Authy 等
用户输入 6 位 TOTP 码

POST /api/auth/mfa/verify (body: {"code": "123456"}, Authorization: Bearer mfaToken)
  → 验证 TOTP
  → 通过 → status: Active → 返回 JWT + RefreshToken + 恢复码
  → 失败 → 返回错误，可重试
```
...
用户尝试启用 Passed 策略 (status → Active)
  → 检查该 Trader 在该 Exchange 上同 Symbol 是否已有 Active 策略？
     → 有 → 拒绝并提示冲突
     → 无 → 切换为 Active
...
Docker 容器启动 → ASP.NET Core 启动
  ...
  → 加载配置 + 数据库 migrations
  → 启动 Trading Engine:
     1. 对每个已启用交易所执行订单同步 reconciliation
     2. 建立 WebSocket 连接
     3. 启动策略评估循环
  → 注册健康检查端点
```
...
TradeX/
├── docker-compose.yml
├── Dockerfile                         # 统一构建：.NET SDK → 运行时
├── backend/
│   ├── TradeX.slnx
│   ├── TradeX.sln
│   ├── TradeX.Api/            # ASP.NET Core Web API + Blazor Server
...
│   │   ├── Pages/                       # Blazor Server 页面组件
│   │   ├── Components/                  # Blazor 可复用组件
│   │   ├── Program.cs
│   │   └── Dockerfile
...
| System | POST | /api/system/emergency-stop | admin/super_admin |
...
| AC-07 | 创建策略（关联单交易所、多条件嵌套）→ 提交回测 → 通过 → 启用（同 Symbol 无冲突时）|
| AC-08 | 尝试在同一 Trader + 同一 Exchange + 同 Symbol 启用第二个 Active 策略 → 被拒绝并提示冲突 |
