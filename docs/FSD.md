...
| 活跃策略冲突 | 同一 Trader 在同一 Exchange 上对同一 Pair 存在多个 Active 策略（不允许） |
| 规则快照 | 当前从交易所实时拉取并缓存的 Pair 交易规则 |
...
### 6.5 ExchangePairRuleSnapshot

| 字段 | 类型 | 约束 | 说明 |
|---|---|---|---|
| Id | Guid | PK | |
| ExchangeId | Guid | FK | |
| Pair | string(30) | required | e.g. BTCUSDT |
...
| Pairs | string | default "[]" | 关联交易对 ID 列表 JSON 数组，为空时表示 Exchange/Trader 作用域 |
...
| Id | Guid | PK | |
| ExchangeId | Guid | FK, required | |
| PairName | string(30) | required | e.g. BTCUSDT |
...
策略模板是一个可复用的条件树 + 执行规则配置，策略部署是将模板绑定到具体的 Trader/Exchange/Pair 作用域。
...
```
- 作用域自动推导：`scope` 由后端根据 `Pairs` 和 `exchangeId` 自动计算
...
- `enable: true`:
  ...
  - 约束：同一 Trader 在同一 Exchange 上对同一 Pair 无其他 `Active` 部署
  - 成功：状态 → `Active`
- `enable: false`:
  - 约束：部署状态必须为 `Active`
  - 成功：状态 → `Disabled`
...
- Response: `200` + `{ taskId, status, createdAt, strategyName, Pair, timeframe }`
...
| 交易所币种集中度 | 该 Exchange 上单币种持仓不超过总敞口的 X% | `Exchange.MaxPairConcentration` |

#### 9.5.4 币种级（Pair-Level）

| 风控项 | 说明 | 配置位置 |
|--------|------|---------|
| 单币种总持仓上限 | 该 Pair 在所有策略上的合计持仓数量上限 | `PairRule.MaxTotalPosition` |
| 单币种日交易次数上限 | 该 Pair 每日累计买入次数上限 | `PairRule.MaxDailyTradeCount` |
| 单币种持仓保证金 | 针对同一个 Pair 的同时持仓量在多个关联策略间的累加 | 运行时由 Trading Engine 叠加计算 |
...
| 币种级 | 跳过本轮该 Pair 的买入，记录日志 | 下一评估周期自动重试 |
...
| `CheckPairLevel(Pair)` | 币种级 | 单币种持仓上限、日交易次数 |
...
| `TradeContext` | 交易上下文（Trader/Exchange/Strategy/Pair/Side/Quantity/Price/ExecutionRule） | 下单执行入参 |
...
{
  "type": "order_filled",
  "template": "🟢 {strategyName} | BUY {quantity} {Pair}\nPrice: {price} USDT\nAmount: {amount} USDT\nFee: {fee} {feeAsset}\nTime: {timestamp}"
}
```
...
- 交易对级（Pair）：绑定具体交易对，需选择策略模板 + 交易所 + 交易对列表

**交易对选择表格（仅 Pair 作用域）**：
- 切换交易所后自动调用 `GET /api/exchanges/{id}/Pairs` 实时拉取 USDT 交易对
- 响应包含：`Pair`, `price`, `priceChangePercent`, `volume`, `highPrice`, `lowPrice`, `pricePrecision`, `minNotional`
...
### M2-B 规则与限流
- Rules 拉取接口 + `ExchangePairRuleSnapshot` 存储
...
- 策略状态机 + toggle API（含活跃策略冲突校验：同一 Trader 在同一 Exchange 上对同一 Pair 仅允许一个 Active 策略）
- 策略编辑器前端骨架
...
| 5 | 冷却期的粒度：全局级别 vs per-strategy vs per-Pair | 当前：per-strategy（由 `ExecutionRule.cooldownSeconds` 控制）| Trading |
| 6 | 回测通过标准是否应可配置？ | 当前：硬编码最低门槛；建议 M6 前改为可配置 | System Settings |
| 7 | 限价单支持：首批交易所是否均支持？不符合是否禁止启用策略？ | 建议：M7 调研后统一，M4-C 暂按市价单实现 | Exchange |
| 8 | 是否支持同一个策略多币种？当前 model 支持 `Pairs[]` | 倾向：支持，但条件树中应能引用 `$Pair` 变量 | Strategy |
...
| v1.1 | 2026-04-24 | 领域修正：引入 Trader 核心实体。新增 Trader 数据模型 + CRUD API；Exchange/Strategy/Position/Order 增加 TraderId FK；增加活跃策略冲突约束（同一 Trader 在同一 Exchange 上对同一 Pair 仅一个 Active 策略）；更新权限模型与错误码；里程碑新增 M1-C |
| v1.2 | 2026-04-24 | 领域修正：重构 ExchangeAccount → Exchange，消除「交易所账户」中间抽象。Exchange 直属于 Trader，为用户配置交易所的关键实体。约束范围细化至同一 Trader × 同一 Exchange × 同一 Pair |
...
| v1.8 | 2026-04-27 | 代码实现同步：补充遗漏数据模型 `StrategyDeployment`（§6.12）、`RefreshToken`（§6.13）；补充 User 字段 `Username`（原名 `UserName`）、`Email`、`IsMfaEnabled`、`LastLoginAt`、`IsDeleted`；补充 Order 字段 `PositionId`；补充 Exchange API 端点 `Pairs`/`assets`/`orders`/`toggle`；补充 Auth logout 端点；策略拆分为模板（`/api/strategies`）和部署（`/api/traders/{traderId}/strategies`）两层 API；Orders/Positions 路由修正为 trader 作用域；更新 Casbin 策略表以包含新增端点 |
