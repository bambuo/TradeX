...
const exchangeTypes = ['Binance', 'OKX', 'Gate', 'Bybit', 'HTX']

async function loadAccounts() {
  loading.value = true
  try {
    const { data } = await exchangeAccountsApi.getAll(traderId)
    accounts.value = data
  } finally {
    loading.value = false
  }
...
function openEdit(account: ExchangeAccount) {
...
async function save() {
  if (editId.value) {
    const payload: Record<string, unknown> = { label: formLabel.value }
    ...
    payload.isTestnet = formIsTestnet.value
    await exchangeAccountsApi.update(traderId, editId.value, payload)
  } else {
    await exchangeAccountsApi.create(traderId, {
...
async function remove(id: string) {
  await exchangeAccountsApi.delete(traderId, id)
  await loadAccounts()
}
...
async function testConnection(id: string) {
  ...
  try {
    const { data } = await exchangeAccountsApi.testConnection(traderId, id)
...
<template>
  <div class="exchanges-page">
    <header class="page-header">
      ...
      <button class="btn-primary" @click="openCreate">添加账户</button>
    </header>

    <div v-if="showForm" class="modal-overlay" @click.self="showForm = false">
      <div class="modal">
        <h3>{{ editId ? '编辑账户' : '添加交易所账户' }}</h3>
        <input v-model="formLabel" placeholder="标签（如：币安主账户）" />
        <select v-model="formExchangeType" :disabled="!!editId">
          <option v-for="t in exchangeTypes" :key="t" :value="t">{{ t }}</option>
        </select>
        <input v-model="formApiKey" :placeholder="editId ? '留空则不修改' : 'API Key'" type="password" />
        <input v-model="formSecretKey" :placeholder="editId ? '留空则不修改' : 'Secret Key'" type="password" />
        <input v-model="formPassphrase" placeholder="Passphrase（选填）" type="password" />
    ...
    <table v-else class="table">
      ...
      <tbody>
        ...
        <tr v-if="accounts.length === 0">
          <td colspan="6" class="empty">暂无交易所账户</td>
        </tr>
      </tbody>
    </table>
  </div>
...
.btn-back {
  ...
  padding: 0.25rem 0.75rem; border-radius: 4px; cursor: pointer; font-size: 0.9rem;
}
.btn-primary {
  padding: 0.5rem 1rem;
  background: #38bdf8;
...
.modal {
  background: #1e293b; padding: 2rem; border-radius: 8px;
...
.checkbox-label {
  ...
  color: #e2e8f0; margin-bottom: 1rem; cursor: pointer;
}
