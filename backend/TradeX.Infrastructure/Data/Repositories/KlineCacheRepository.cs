...
{
    public async Task<Candle[]> GetKlinesAsync(Guid exchangeId, string pair, string timeframe, DateTime startAt, DateTime endAt, CancellationToken ct = default)
    {
        ...
        return entities.Select(e => new Candle(e.Timestamp, e.Open, e.High, e.Low, e.Close, e.Volume)).ToArray();
    }

    public async Task SaveKlinesAsync(Guid exchangeId, string pair, string timeframe, Candle[] candles, CancellationToken ct = default)
    {
        ...
        var newEntities = candles
            ...
            .Select(c => KlineCacheEntity.FromCandle(exchangeId, pair, timeframe, c))
            .ToArray();
