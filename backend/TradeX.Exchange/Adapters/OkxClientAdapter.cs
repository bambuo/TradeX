...
public class OkxClientAdapter : IExchangeClient
{
    ...
    public async Task<ExchangeOrderDto[]> GetOrderHistoryAsync(int limit = 100, CancellationToken ct = default)
    {
        ...
        foreach (var asset in bals.Keys.Where(a => a != "USDT").Select(a => $"{a}-USDT"))
        {
            ...
            var r = await _client.UnifiedApi.Trading.GetOrderHistoryAsync(InstrumentType.Spot, symbol: asset, limit: limit, ct: ct);
    ...
    {
        if (!_hasCredentials) return [];
        var r = await _client.UnifiedApi.Trading.GetOrderHistoryAsync(InstrumentType.Spot, symbol: pair, limit: limit, ct: ct);
        if (!r.Success) return [];
        return r.Data.Select(MapOrder).ToArray();
    }
