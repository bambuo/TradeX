...
public class GateIoClientAdapter : IExchangeClient
{
    ...
    public async Task<ExchangeOrderDto[]> GetOrderHistoryAsync(int limit = 100, CancellationToken ct = default)
    {
        ...
        foreach (var asset in bals.Keys.Where(a => a != "USDT").Select(a => $"{a}_USDT"))
        {
            ...
            var r = await _client.SpotApi.Trading.GetOrdersAsync(false, asset, limit: limit, ct: ct);
    ...
    {
        if (!_hasCredentials) return [];
        var r = await _client.SpotApi.Trading.GetOrdersAsync(false, pair, limit: limit, ct: ct);
        if (!r.Success) return [];
