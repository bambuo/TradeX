...
public class PositionReconcilerTests
{
    ...
    private static (PositionReconciler reconciler, IDomainEventBus eventBus, IExchangeClient client)
        Build(RiskSettings settings, decimal localQty, decimal exchangeQty)
    {
        ...
        exchangeRepo.GetAllEnabledAsync(Arg.Any<CancellationToken>()).Returns([
            new TradeX.Core.Models.Exchange { Id = exchangeId, Type = ExchangeType.Binance,
        ...
        var reconciler = new PositionReconciler(exchangeRepo, positionRepo, clientFactory, encryption,
            eventBus, new TradeXMetrics(), Options.Create(settings), Substitute.For<ILogger<PositionReconciler>>());

        return (reconciler, eventBus, client);
    }
    ...
    public async Task Overstatement_BeyondTolerance_EmitsCriticalAlert()
    {
        ...
        var (reconciler, eventBus, _) = Build(new RiskSettings { PositionDriftTolerancePercent = 1m },
            localQty: 1.0m, exchangeQty: 0.5m);

        PositionDriftDetectedPayload? captured = null;
        await eventBus.PublishAsync(Arg.Do<PositionDriftDetectedPayload>(p => captured = p), Arg.Any<CancellationToken>());

        var count = await reconciler.ReconcilePositionsAsync();

        Assert.Equal(1, count);
        Assert.NotNull(captured);
        Assert.Contains("Critical", captured!.Severity);
        Assert.Equal(1.0m, captured.LocalQuantity);
        Assert.Equal(0.5m, captured.ExchangeQuantity);
    }
    ...
    public async Task WithinTolerance_NoAlert()
    {
        ...
        await eventBus.DidNotReceive().PublishAsync(Arg.Any<PositionDriftDetectedPayload>(), Arg.Any<CancellationToken>());
    }
    ...
    {
        var (reconciler, eventBus, _) = Build(
        ...
        Assert.Equal(1, count);
        Assert.Contains("Warning", captured!.Severity);
    }

    [Fact]
    public async Task Disabled_NoOp()
    {
        var (reconciler, eventBus, client) = Build(
