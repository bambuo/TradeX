        ...
        CancellationToken ct = default, Guid? traceId = null)
    {
        await hubContext.Clients.Group($"trader_{traderId}")
            .SendAsync(TradingHub.PositionUpdated, new PositionUpdatedEvent(
                ...
                entryPrice, unrealizedPnl, realizedPnl, status, updatedAtUtc, traceId ?? Guid.Empty), ct);
    }
        ...
        CancellationToken ct = default, Guid? traceId = null)
    {
        await hubContext.Clients.Group($"trader_{traderId}")
            .SendAsync(TradingHub.OrderPlaced, new OrderPlacedEvent(
                ...
                side, type, status, quantity, placedAtUtc, traceId ?? Guid.Empty), ct);
    }
        ...
        CancellationToken ct = default, Guid? traceId = null)
    {
        await hubContext.Clients.Group($"trader_{traderId}")
            .SendAsync(TradingHub.BindingStatusChanged, new BindingStatusChangedEvent(
                strategyId, traderId, oldStatus, newStatus, reason, DateTime.UtcNow, traceId ?? Guid.Empty), ct);
    }
        ...
        CancellationToken ct = default, Guid? traceId = null)
    {
        await hubContext.Clients.Group($"trader_{traderId}")
            .SendAsync(TradingHub.RiskAlert, new RiskAlertEvent(
                Guid.NewGuid(), level, category, traderId, strategyId, message, DateTime.UtcNow, traceId ?? Guid.Empty), ct);
    }
        ...
        CancellationToken ct = default, Guid? traceId = null)
    {
        await hubContext.Clients.Group($"trader_{traderId}")
            .SendAsync(TradingHub.DashboardSummary, new DashboardSummaryEvent(
                totalPnl, totalPositions, activeStrategies, dailyPnl, winRate, lastUpdateAtUtc, traceId ?? Guid.Empty), ct);
    }
        ...
        CancellationToken ct = default, Guid? traceId = null)
    {
        await hubContext.Clients.Group($"trader_{traderId}")
            .SendAsync(TradingHub.ExchangeConnectionChanged, new ExchangeConnectionChangedEvent(
                exchangeId, traderId, oldStatus, newStatus, errorMessage, DateTime.UtcNow, traceId ?? Guid.Empty), ct);
    }
}
