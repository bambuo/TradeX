...
public interface IExchangeAccountRepository
{
    ...
    Task<List<ExchangeAccount>> GetByTraderIdAsync(Guid traderId, CancellationToken ct = default);
    Task<List<ExchangeAccount>> GetAllEnabledAsync(CancellationToken ct = default);
    Task<bool> IsNameUniqueAsync(Guid traderId, string name, CancellationToken ct = default);
