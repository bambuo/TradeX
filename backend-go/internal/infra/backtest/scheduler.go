...
func (s *BacktestScheduler) Run(ctx context.Context, cfg SchedulerConfig, guards ...guard.Guard) {
...
func (s *BacktestScheduler) recoverStuckTasks(ctx context.Context) {
	...
	if err != nil {
		...
		for _, task := range running {
			runningCount++
			task.Status = bt.TaskStatusPending
			task.Phase = nil
			if err := s.repo.SaveTask(context.Background(), task); err != nil {
...
func (s *BacktestScheduler) executeTask(ctx context.Context, taskID uuid.UUID, cfg SchedulerConfig) {
	...
	advance := func(from bt.BacktestTaskStatus, phase bt.BacktestPhase) bool {
		ok, _ := s.repo.ClaimTask(taskCtx, taskID, from, phase)
		return ok
	}

	log.Info().Interface("event", bt.BacktestStartedEvent{
		TaskID: taskID, StrategyName: task.StrategyName, Pair: task.Pair, Timestamp: time.Now(),
	}).Msg("回测已开始")
	...
	if err != nil {
		s.failTask(task, fmt.Sprintf("load strategy: %s", err.Error()), log)
		return
	}
	...
	out.Result.FillMeta(strategy.Name, task.Pair, task.Timeframe, task.StartAt, task.EndAt, task.InitialCapital)
	...
	err = s.repo.ExecuteInTransaction(taskCtx, func(txRepo bt.BacktestRepository) error {
		...
		completeTask, err := txRepo.GetTask(taskCtx, taskID)
		if err != nil {
			return err
		}
		...
		return txRepo.SaveTask(taskCtx, completeTask)
	})
	if err != nil {
...
func (s *BacktestScheduler) failTask(task *bt.BacktestTask, reason string, log zerolog.Logger) {
	log.Error().Str("reason", reason).Msg("回测失败")
	log.Info().Interface("event", bt.BacktestFailedEvent{
		TaskID: task.ID, StrategyName: task.StrategyName, Pair: task.Pair, Reason: reason, Timestamp: time.Now(),
	}).Msg("回测失败（事件）")
	if err := task.Fail(); err != nil {
		log.Warn().Err(err).Msg("标记任务失败时状态异常")
	}
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	if err := s.repo.SaveTask(ctx, task); err != nil {
		log.Warn().Err(err).Msg("保存任务失败状态出错")
	}
}
...
func (s *BacktestScheduler) fetchKlines(ctx context.Context, task *bt.BacktestTask) ([]domain.Kline, error) {
	if s.klineCache != nil {
